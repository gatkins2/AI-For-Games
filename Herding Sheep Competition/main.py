...
def buildGates(graph):
    ...
    if finalGate[0][Y] == finalGate[1][Y]:
        ...
        if finalGate[0][X] > finalGate[1][X]:
            ...
            penBounds.append(pygame.Rect(finalGate[1][X] + GRID_SIZE // 2, finalGate[1][Y] - GRID_SIZE // 2,
                                         finalGate[0][X] - finalGate[1][X] - GRID_SIZE, GRID_SIZE))

            # Create the pen's "inside" collider
            penBounds.append(
                pygame.Rect(finalGate[1][X] + GRID_SIZE // 2, finalGate[1][Y] - PEN_DEPTH + GRID_SIZE // 2,
                            finalGate[0][X] - finalGate[1][X] - GRID_SIZE, PEN_DEPTH - GRID_SIZE * 2))
        else:
            direction = 1
            # Create the gate's "entrance" collider
            penBounds.append(pygame.Rect(finalGate[0][X] + GRID_SIZE // 2, finalGate[1][Y] - GRID_SIZE // 2,
                                         finalGate[1][X] - finalGate[0][X] - GRID_SIZE, GRID_SIZE))

            # Create the pen's "inside" collider
            penBounds.append(pygame.Rect(finalGate[0][X] + GRID_SIZE // 2, finalGate[0][Y] + GRID_SIZE // 2,
                                         finalGate[1][X] - finalGate[0][X] - GRID_SIZE, PEN_DEPTH - GRID_SIZE * 2))

        # Draw the two sides of the pen
        for y in range(finalGate[0][Y] + direction * GRID_SIZE, finalGate[0][Y] + direction * PEN_DEPTH,
                       direction * GRID_SIZE):
